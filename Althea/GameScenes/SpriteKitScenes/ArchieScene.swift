...
class ArchieScene: SKScene, SKPhysicsContactDelegate, ObservableObject {
    ...
    var treasureBigArea1 = SKSpriteNode()// area luar treasure, beep sound play tp blm cepet.
    var treasureBigArea2 = SKSpriteNode()// area luar treasure, beep sound play tp blm cepet.
    var treasureBigArea3 = SKSpriteNode()// area luar treasure, beep sound play tp blm cepet.
    var treasureBigArea4 = SKSpriteNode()// area luar treasure, beep sound play tp blm cepet.
    var treasureBigArea5 = SKSpriteNode()// area luar treasure, beep sound play tp blm cepet.
    var treasureBigArea6 = SKSpriteNode()// area luar treasure, beep sound play tp blm cepet.
    var treasureBigArea7 = SKSpriteNode()// area luar treasure, beep sound play tp blm cepet.
    var treasureLocation = SKSpriteNode() // area treasure yg bisa di-dig, beep sound makin cepet.
    var treasureLocation1 = SKSpriteNode() // area treasure yg bisa di-dig, beep sound makin cepet.
    var treasureLocation2 = SKSpriteNode() // area treasure yg bisa di-dig, beep sound makin cepet.
    var treasureLocation3 = SKSpriteNode() // area treasure yg bisa di-dig, beep sound makin cepet.
    var treasureLocation4 = SKSpriteNode() // area treasure yg bisa di-dig, beep sound makin cepet.
    var treasureLocation5 = SKSpriteNode() // area treasure yg bisa di-dig, beep sound makin cepet.
    var treasureLocation6 = SKSpriteNode() // area treasure yg bisa di-dig, beep sound makin cepet.
    var treasureLocation7 = SKSpriteNode() // area treasure yg bisa di-dig, beep sound makin cepet.
    
    @Published var isPlayerInTreasureLocation = false
    
    var itemsCollected: [String: Int] = [:]
    @Published var isMetalDetectorActive: Bool = false
    ...
    var isPlayerInsideTreasureArea = false
...
//        bomb.name = "Bomb"
    ...
    func setupTreasureNode1() {
        treasureBigArea1 = childNode(withName: "treasureBigArea1") as! SKSpriteNode
        treasureBigArea1.name = "TreasureBigArea1" // add to PhysicsCategory


        treasureBigArea1.physicsBody = SKPhysicsBody(rectangleOf: treasureBigArea1.size)
        treasureBigArea1.physicsBody?.isDynamic = false
        treasureBigArea1.physicsBody?.categoryBitMask = PhysicsCategory.TreasureBigArea
        treasureBigArea1.alpha = 0
        treasureBigArea1.physicsBody?.contactTestBitMask = PhysicsCategory.Player
        treasureBigArea1.physicsBody?.collisionBitMask = 0


        treasureLocation1 = childNode(withName: "treasureLocation1") as! SKSpriteNode
        treasureLocation1.name = "TreasureLocation1" // add to PhysicsCategory

        treasureLocation1.physicsBody = SKPhysicsBody(rectangleOf: treasureLocation1.size)
        treasureLocation1.physicsBody?.isDynamic = false
        treasureLocation1.physicsBody?.categoryBitMask = PhysicsCategory.TreasureLocation
        treasureLocation1.alpha = 0
        treasureLocation1.physicsBody?.contactTestBitMask = PhysicsCategory.Player
        treasureLocation1.physicsBody?.collisionBitMask = 0
    ...
    func setupPlayerNode() {
        ...
        characterSupply.physicsBody?.contactTestBitMask = PhysicsCategory.CornerArea | PhysicsCategory.MiddleArea | PhysicsCategory.Apple | PhysicsCategory.Wheat | PhysicsCategory.Rock | PhysicsCategory.Pineapple | PhysicsCategory.Wood | PhysicsCategory.Bomb | PhysicsCategory.TreasureLocation | PhysicsCategory.TreasureBigArea
...
//            contact.bodyA.node?.removeFromParent()
        ...
        if (contact.bodyA.categoryBitMask == PhysicsCategory.Player && contact.bodyB.categoryBitMask == PhysicsCategory.TreasureLocation) ||
            ...
            if let playerNode = contact.bodyA.categoryBitMask == PhysicsCategory.Player ? contact.bodyA.node : contact.bodyB.node {
                ...
                stopBeepSound(on: playerNode)
                if isMetalDetectorActive && isPlayerInTreasureLocation {
                    playBoopSound(on: playerNode, withSpeedMultiplier: 10.0)
                }
            }
            startHaptic()
            isPlayerInTreasureLocation = true
    ...
    func didEnd(_ contact: SKPhysicsContact) {
        if (contact.bodyA.categoryBitMask == PhysicsCategory.Player && contact.bodyB.categoryBitMask == PhysicsCategory.TreasureBigArea) ||
           (contact.bodyA.categoryBitMask == PhysicsCategory.TreasureBigArea && contact.bodyB.categoryBitMask == PhysicsCategory.Player) {
            ...
            print("Player left the treasure area")
            
            isPlayerInsideTreasureArea = false
            
            // Stop the beep sound when the player leaves the area
            let playerNode = contact.bodyA.categoryBitMask == PhysicsCategory.Player ? contact.bodyA.node : contact.bodyB.node
            stopBeepSound(on: playerNode!)
            
            // You can also stop the haptic feedback here if you want it to end when the player leaves the area.
             stopHaptic()
        }
    }
    ...
    func stopHaptic() {
        ...
        impactFeedbackGenerator.impactOccurred(intensity: 0) // Stop the haptic feedback
    ...
    func playBoopSound(on playerNode: SKNode, withSpeedMultiplier speedMultiplier: CGFloat) {
        let beepSound = SKAction.playSoundFileNamed("beep.mp3", waitForCompletion: true)

        let speedAction = SKAction.speed(to: speedMultiplier, duration: 0)

        let repeatBeepSound = SKAction.repeatForever(SKAction.sequence([speedAction, beepSound]))
        playerNode.run(repeatBeepSound, withKey: "boopSound")
    }
    
    func stopBeepSound(on playerNode: SKNode) {
        // Assuming you have a reference to the playerNode
        // let playerNode = ...
        
        // Remove the 'beepSound' action from the playerNode using its key
        playerNode.removeAction(forKey: "beepSound")
    }
